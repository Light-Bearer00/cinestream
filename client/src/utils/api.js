...
export const movieApi = {
  ...
  search:    (query, filters = {}) => api.get('/movies', { params: { search: query, ...filters } }),
};
...
export const tvApi = {
  ...
  getEpisode: (id, season, episode)  => api.get(`/tv/${id}/season/${season}/episode/${episode}`),
...
export const userApi = {
  ...
  addToHistory:   (movieId, progress)   => api.post(`/users/history/${movieId}`, { progress }),
};

// ─── Admin API ────────────────────────────────────────────────────────────────
export const adminApi = {
  getMovies:      ()           => api.get('/admin/movies'),
  fetchMovieInfo: (title, year)=> api.get('/admin/fetch-movie-info', { params: { title, year } }),
  createMovie:    (formData)   => api.post('/admin/movies', formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
  updateMovie:    (id, formData)=> api.put(`/admin/movies/${id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } }),
...
export const tvScraperApi = {
  ...
  details:   (tmdbId, withSeasons = false)  => api.get(`/tv-scraper/details/${tmdbId}`, { params: { seasons: withSeasons } }),
